...
class TestMain(TestCase):
    def test_main_get_parser_args_match_bgkube_attributes(self):
        args = [arg.dest for arg in get_parser()._actions if arg.dest not in ['help', 'command']]
        options = Mock(**{arg: get_random_str() for arg in args})

        for name in args:
            self.assertIsNotNone(getattr(BgKube(options), name, None), name)
